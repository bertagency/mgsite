...
let NavHideHome = document.querySelectorAll("[menu='hide']");
let NavShowHome = document.querySelectorAll("[menu='show']");
...
function openMenuContainer() {
  ...
  tl.to(menuBottom, {
    ...
    .to(NavHideHome, { opacity: 1, duration: 0 }, 0)
    .to(NavShowHome, { opacity: 0, duration: 0 }, 0);
}

function openMenuContainerBack() {
  menuBottom.style.height = "0";
  menuBottom.style.display = "none";
  let globalNavColor = document.querySelector('[global="nav"]');

  // Check if the element exists in the document
  if (globalNavColor) {
    let style = window.getComputedStyle(globalNavColor);

    // Now, style contains all the computed styles of the element
    let bgColor = style.backgroundColor; // get the background color

    if (bgColor === "rgba(0, 0, 0, 0)") {
      let tl = gsap.timeline();
      // Assuming NavHideHome and NavShowHome are defined and valid selectors or elements.
      tl.to(NavHideHome, { opacity: 0, duration: 0 }, 0).to(
        NavShowHome,
    ...
    console.error("Element with 'global=nav' not found");
  }
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let NavHide = globalNav.querySelectorAll("[scrollnav='hide']");
  let NavShow = globalNav.querySelectorAll("[scrollnav='show']");
  let NavLinks = globalNav.querySelectorAll("[data-link='menu']");
  let searchIcon = globalNav.querySelector("[menu='search-icon']");
